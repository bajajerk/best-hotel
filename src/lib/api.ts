...
export interface HotelDetail {
  /** Hotel master UUID — canonical id surfaced by the rates endpoint. */
  master_id?: string;
 ...
 *  UUID, slug-shortid, or legacy tj id). The backend resolver normalises all
 *  forms via `_resolve_tj_from_path_id`. The canonical caller passes the same
 *  string that lives in the URL. Any failure is non-fatal — pages should fall
 *  back to the rates response, which carries the same meta. */
export async function fetchHotelDetail(pathId: string): Promise<HotelDetail> {
  const res = await fetch(`${API_BASE}/api/hotels/${pathId}`);
...
export type RatesResponse = {
  /** Hotel meta returned by GET /api/hotels/{path}/rates. The path token can
...
export type NoMatchError = { error: "no_tripjack_match"; path_id: string };

/**
 * Fetch live rates for the hotel at the given URL path token (slug-shortid,
 * short_id, master UUID, or legacy tj id). Backend's `_resolve_tj_from_path_id`
 * normalises before query.
 */
export async function fetchHotelRates(
  ...
  const res = await fetch(`${API_BASE}/api/hotels/${pathId}/rates?${params}`, { cache: "no-store" });
  if (res.status === 404) {
    const body = await res.json().catch(() => ({}));
    return { error: "no_tripjack_match", path_id: String(pathId), ...body };
  }
...
export type BatchRatesResponse = {
  checkin: string;
  checkout: string;
  /** Keyed by hotel master_id (UUID). */
  results: Record<string, BatchRate>;
  /** Hotel master_ids that did not match a TripJack property. */
  unmatched_ids: string[];
};

export async function fetchBatchRates(
  hotelMasterIds: string[],
  checkin: string,
  checkout: string,
  adults: number = 2,
  children: number = 0
...
export interface CreateBookingBody {
  /** Hotel master UUID — canonical id accepted by POST /api/bookings post-Phase B. */
  hotel_master_id: string;
  provider: string;
  checkin: string;
  checkout: string;
  currency: string;
