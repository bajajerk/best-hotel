...
// ---------------------------------------------------------------------------
// Sample data — 4 cards, max 2 Voyagr Picks
// ---------------------------------------------------------------------------
const SAMPLE_CARDS: FeaturedHotelCard[] = [
  ...
  },
  {
    ...
    citySlug: "tokyo",
    editorial: "Where minimalist serenity floats above the city",
    publicRate: 680,
    memberRate: 475,
...
function DestinationCard({
  ...
  card: FeaturedHotelCard;
  gated?: boolean;
  onGatedRateClick?: (hotelName: string) => void;
}) {
      ...
      <div style={{ padding: "20px" }}>
        ...
        <div
          ...
          style={{
            ...
            alignItems: "baseline",
            gap: "10px",
            cursor: gated ? "pointer" : undefined,
          }}
          data-hotel-name={card.name}
        >
          ...
          {gated ? (
            /* Gated: show blurred rate + "See rate" prompt */
            <span
              style={{
                fontFamily: "var(--font-display)",
                fontSize: "22px",
  ...
  if (gated) {
    return (
      <div
        ...
        onClick={() => onGatedRateClick?.(card.name)}
      >
        {cardContent}
      </div>
    );
...
export default function FeaturedDestinationsGrid({
  ...
  const [gatedHotelName, setGatedHotelName] = useState("");

  const handleGatedRateClick = (hotelName: string) => {
    setGatedHotelName(hotelName);
    setModalOpen(true);
  };
  ...
  return (
    ...
    >
      <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
        ...
        {/* 4-up desktop, 2-up tablet, 1-up mobile, gap 2px */}
        <div
        ...
        >
          {cards.map((card, index) => (
            <DestinationCard
              ...
              card={card}
              gated={index >= 3}
              onGatedRateClick={handleGatedRateClick}
            />
          ))}
        </div>
      </div>

      {/* Signup gate modal — triggered by 4th+ card rate click */}
      <SignupGateModal
        open={modalOpen}
        onClose={() => setModalOpen(false)}
        hotelName={gatedHotelName}
      />
